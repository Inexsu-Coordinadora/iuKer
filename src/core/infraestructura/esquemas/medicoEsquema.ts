...
export const crearMedicoEsquema = z.object({
  tarjetaProfesional: z
    .string()
    .min(3, {
      message: 'La tarjeta profesional debe tener al menos 3 caracteres',
    })
    .max(50, {
      message: 'La tarjeta profesional no puede superar los 50 caracteres',
    }),
  ...
  fechaNacimiento: z.string().refine((value) => !isNaN(Date.parse(value)), {
  ...
  sexo: z
    ...
    .refine((value) => ['M', 'F', 'O'].includes(value.toUpperCase()), {
  ...
  telefono: z.string().regex(/^[0-9]{7,10}$/, {
